...
export function createGameBridgeRouter({ gameBridgeService, gamePresenceService, recruitmentService, robloxApiKey }) {
  const router = Router();
  const auth = createRobloxAuth(robloxApiKey);
  const controller = createGameBridgeController(gameBridgeService, gamePresenceService, recruitmentService);
