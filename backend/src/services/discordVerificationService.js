...
export class DiscordVerificationService {
  ...
  async syncMemberFromCache(discordUserId) {
    const cached = await this.database.getVerificationProfileByDiscordUserId(discordUserId);
    if (!cached?.link || !cached.profile) {
      const error = new Error("O usuário precisa estar verificado e ter um perfil sincronizado pelo jogo.");
      error.code = "PROFILE_NOT_SYNCED";
      throw error;
    }

    const guild = await this.client.guilds.fetch(this.guildId);
    const member = await guild.members.fetch(String(discordUserId));
    const roleResult = await this.roleSyncService.sync(member, cached.profile);
    return { ...cached, roleResult, member };
  }
