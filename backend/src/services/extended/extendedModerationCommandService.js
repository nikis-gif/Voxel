...
export class ExtendedModerationCommandService {
  ...
  async handleCommand(interaction) {
    ...
    switch (name) {
      ...
      case "sync-me": return this.handleSyncMe(interaction);
  ...
  async handleSyncMe(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const result = await this.verificationService.syncMemberFromCache(interaction.user.id);
    const added = result.roleResult?.added ?? [];
    const removed = result.roleResult?.removed ?? [];
    const active = result.roleResult?.active ?? [];

    await interaction.editReply({
      embeds: [this.baseEmbed(
        "Sincronização concluída",
        "Seus cargos e apelido foram comparados com o perfil mais recente recebido do jogo."
      ).addFields(
        { name: "Cargos ativos", value: active.length ? active.map((name) => `• ${name}`).join("\n") : "Nenhum cargo gerenciado ativo.", inline: false },
        { name: "Adicionados", value: added.length ? added.join(", ") : "Nenhum", inline: true },
        { name: "Removidos", value: removed.length ? removed.join(", ") : "Nenhum", inline: true }
      ).setTimestamp()]
    });
  }
