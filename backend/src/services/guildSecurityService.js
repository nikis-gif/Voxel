...
export class GuildSecurityService {
  constructor({ client, guildId, database, supportOwnerId }) {
    ...
    this.antiRaidEnabled = true;
    this.antiNukeEnabled = true;
    this.antiRaidThreshold = VOXEL_SECURITY_CONFIG.antiRaid.joinThreshold;
    this.antiNukeThreshold = VOXEL_SECURITY_CONFIG.antiNuke.scoreThreshold;
  ...
  async init() {
    ...
    const [storedRaidUntil, antiRaidEnabled, antiNukeEnabled, antiRaidThreshold, antiNukeThreshold, whitelist] = await Promise.all([
      this.database.getSecurityState("raidModeUntil"),
      this.database.getSecurityState("antiRaidEnabled"),
      this.database.getSecurityState("antiNukeEnabled"),
      this.database.getSecurityState("antiRaidThreshold"),
      this.database.getSecurityState("antiNukeThreshold"),
      this.database.getSecurityWhitelist()
    ]);
    if (Number(storedRaidUntil ?? 0) > now()) this.raidModeUntil = Number(storedRaidUntil);
    if (typeof antiRaidEnabled === "boolean") this.antiRaidEnabled = antiRaidEnabled;
    if (typeof antiNukeEnabled === "boolean") this.antiNukeEnabled = antiNukeEnabled;
    if (Number.isInteger(Number(antiRaidThreshold))) this.antiRaidThreshold = Math.max(3, Number(antiRaidThreshold));
    if (Number.isInteger(Number(antiNukeThreshold))) this.antiNukeThreshold = Math.max(3, Number(antiNukeThreshold));
    this.whitelist = new Set((whitelist ?? []).map(String));
  ...
  async handleJoin(member) {
    if (!this.antiRaidEnabled) return;
    ...
    if (this.joinEvents.length >= this.antiRaidThreshold && this.raidModeUntil <= timestamp) {
  ...
  isTrustedExecutor(guild, executorId) {
    ...
    return this.ownerIds.has(String(executorId)) || this.whitelist.has(String(executorId));
  }
  ...
  async handleAuditEntry(entry, guild) {
    if (!this.antiNukeEnabled) return;
    ...
    const config = VOXEL_SECURITY_CONFIG.antiNuke;
    if (score < this.antiNukeThreshold) return;
  ...
  async getStatus() {
    return {
      antiRaidEnabled: this.antiRaidEnabled,
      antiNukeEnabled: this.antiNukeEnabled,
      antiRaidThreshold: this.antiRaidThreshold,
      antiNukeThreshold: this.antiNukeThreshold,
      raidModeUntil: this.raidModeUntil,
      whitelist: [...this.whitelist]
    };
  }

  async configureAntiRaid({ enabled = null, threshold = null } = {}) {
    if (typeof enabled === "boolean") {
      this.antiRaidEnabled = enabled;
      await this.database.setSecurityState("antiRaidEnabled", enabled);
      if (!enabled) {
        this.raidModeUntil = 0;
        await this.database.setSecurityState("raidModeUntil", 0);
      }
    }
    if (threshold != null) {
      this.antiRaidThreshold = Math.max(3, Math.min(50, Math.floor(Number(threshold))));
      await this.database.setSecurityState("antiRaidThreshold", this.antiRaidThreshold);
    }
    return this.getStatus();
  }

  async configureAntiNuke({ enabled = null, threshold = null } = {}) {
    if (typeof enabled === "boolean") {
      this.antiNukeEnabled = enabled;
      await this.database.setSecurityState("antiNukeEnabled", enabled);
    }
    if (threshold != null) {
      this.antiNukeThreshold = Math.max(3, Math.min(30, Math.floor(Number(threshold))));
      await this.database.setSecurityState("antiNukeThreshold", this.antiNukeThreshold);
    }
    return this.getStatus();
  }

  async setWhitelist(discordUserId, enabled) {
    const id = String(discordUserId);
    if (enabled) this.whitelist.add(id);
    else this.whitelist.delete(id);
    await this.database.setSecurityWhitelist(id, enabled);
