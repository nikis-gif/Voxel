...
export class RecruitmentService {
  ...
  async submit({ candidateToken, sessionId, answers }) {
    ...
    const sessionRef = this.sessionsRef.child(String(sessionId ?? ""));
    const timestamp = now();
    const transaction = await sessionRef.transaction((current) => {
      if (!current || typeof current !== "object") return;
      if (String(current.candidateKey ?? "") !== key) return;
      if (Number(current.expiresAt ?? 0) <= timestamp) return;
      if (current.submittedAt) return;
      return { ...current, submittedAt: timestamp };
    });

    if (!transaction.committed) {
      throw createError("Esta prova expirou ou já foi enviada. Inicie uma nova tentativa.", 409);
    }
  ...
  async #completeEnrollment(candidateKey, candidate) {
    try {
      ...
      return {
        ...
        message: "Sua prova foi aprovada, mas o jogo ainda não confirmou a entrada. Mantenha sua conta online no jogo e tente concluir novamente."
      };
    }
  }
}
