  ...
  async reserve({ code, robloxUserId }) {
    ...
    if (!reserved?.reservationToken) {
      const [current, lock] = await Promise.all([
        this.database.getRewardByCode(normalizedCode),
        this.database.getRewardReservation(normalizedCode)
      ]);
      console.warn(
        `[reward] Reservation denied for ${normalizedCode}: requester=${robloxUserId}, `
        + `rewardOwner=${current?.robloxUserId ?? "none"}, lockOwner=${lock?.robloxUserId ?? "none"}, `
        + `lockExpiresAt=${lock?.expiresAt ?? "none"}.`
      );
