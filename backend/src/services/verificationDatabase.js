...
export class VerificationDatabase {
  ...
  async reserveRewardCode({ code, robloxUserId, reservationToken, reservationTtlMs }) {
    ...
    const result = await ref.transaction((current) => {
      ...
      if (!reward) return;
      if (reward.robloxUserId && reward.robloxUserId !== Number(robloxUserId)) return;
      if (reward.consumedAt || reward.expiresAt <= now) return;

      const requesterId = Number(robloxUserId);
      const activeReservation = Boolean(
        reward.reservationToken
        && reward.reservedAt
        && reward.reservedAt > staleBefore
      );

      if (activeReservation) {
        // Same-player retries must renew the lease so Firebase performs a real write.
        // This also recovers codes that were left locked by the previous backend version.
        if (reward.robloxUserId === requesterId || reward.robloxUserId == null) {
          return {
            ...reward,
            robloxUserId: reward.robloxUserId ?? requesterId,
            reservedAt: now
          };
        }
