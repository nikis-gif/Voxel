...
export class VerificationDatabase {
  ...
  async listVerified({ limit = 1000 } = {}) {
    const snapshot = await this.linksRef.child("byDiscord").get();
  ...
  async removeWarning(discordUserId, warningId) {
    const ref = this.warningsRef.child(`${String(discordUserId)}/${String(warningId)}`);
    const snapshot = await ref.get();
    const warning = normalizeWarning(String(warningId), snapshot.val());
    if (!warning) return null;
    await ref.remove();
    return warning;
  }

  async resetWarnings(discordUserId) {
    const ref = this.warningsRef.child(String(discordUserId));
    const snapshot = await ref.get();
  ...
  async updateTicket(discordUserId, patch) {
    const ref = this.ticketsRef.child(String(discordUserId));
    const snapshot = await ref.get();
    const current = snapshot.val();
    if (!current || typeof current !== "object") return null;
    const next = { ...current, ...clone(patch), updatedAt: Date.now() };
    await ref.set(next);
    return next;
  }

  async getTicketByChannelId(channelId) {
    const snapshot = await this.ticketsRef.get();
    for (const [discordUserId, value] of objectValues(snapshot.val())) {
      if (value?.channelId === String(channelId)) return { ...clone(value), discordUserId: String(discordUserId) };
    }
    return null;
  }
  ...
  async listOpenTickets() {
    const snapshot = await this.ticketsRef.get();
    return objectValues(snapshot.val())
      .map(([discordUserId, value]) => ({ ...clone(value), discordUserId: String(discordUserId) }))
      .filter((value) => value.channelId && !value.closedAt)
  ...
  async listRewardsForDiscord(discordUserId, { limit = 20 } = {}) {
    const snapshot = await this.rewardsRef.child(`byDiscord/${String(discordUserId)}`).get();
    return objectValues(snapshot.val())
      .map(([, value]) => normalizeReward(value))
      .filter(Boolean)
  ...
  async revokeRewardCode(code) {
    const normalizedCode = String(code).trim().toUpperCase();
    const reward = await this.getRewardByCode(normalizedCode);
    if (!reward || reward.consumedAt) return null;
    const updates = {
      [`codes/${normalizedCode}`]: null,
      [`reservations/${normalizedCode}`]: null
    };
    if (reward.discordUserId) updates[`byDiscord/${reward.discordUserId}/${normalizedCode}`] = null;
  ...
  async listSecurityIncidents({ limit = 20 } = {}) {
    const snapshot = await this.securityRef.child("incidents").get();
    return objectValues(snapshot.val())
      .map(([, value]) => value)
  ...
  async getSecurityIncident(id) {
    const snapshot = await this.securityRef.child(`incidents/${String(id)}`).get();
    return snapshot.val() ?? null;
  }

  async getSecurityWhitelist() {
    const snapshot = await this.securityRef.child("whitelist").get();
    return Object.keys(snapshot.val() ?? {});
  }

  async setSecurityWhitelist(discordUserId, enabled) {
    const ref = this.securityRef.child(`whitelist/${String(discordUserId)}`);
    if (enabled) await ref.set({ addedAt: Date.now() });
